...
packs = sort(c("grid", "gridExtra", "gtable", "multipanelfigure", "RColorBrewer", "tidyverse", "ggplot2", "libcurl" "cowplot", "egg", "ggpubr", "ggplotify", "RColorBrewer", "tidyverse"))

# require(remotes)
# install.packages("curl")
# install_version("ggplot2", version = "3.5.0")
# install_version("cowplot", version = "1.1.3")
# install_version("grid", version = "4.2.3")
# install_version("gridExtra", version = "2.3")
# install_version("gtable", version = "0.3.4")
# install_version("egg", version = "0.4.5")
# install_version("ggpubr", version = "0.6.0")
# install_version("ggplotify", version = "0.1.2")
...
dt = data.frame(x = 1:10, y = 1:10)
p1 = ggplot(data = dt, aes(x = 1:10, y = 1:10)) + geom_point()

# https://cran.r-project.org/web/packages/egg/vignettes/Ecosystem.html

layout_matrix = rbind(c(1, 2, 3), c(1, 4, 4))

# Option 1: x only, no hjust
# Option 1: hjust only
egg::ggarrange(p1, p1 + theme(axis.title.y = element_blank()), 
               p1, p1 + theme(axis.title.y = element_blank()), 
               ncol = 2, nrow = 2, 
               widths = c(1, 1), heights = c(1, 1), 
               labels = c("A", "B", "C", "D"),
               draw = T, debug = T,
               label.args = list(gp = grid::gpar(fontfamily = "sans", 
                                           fontsize = 12, fontface = "bold"), 
                                 x = unit(0, "cm"), vjust = 1, hjust = 1))

# The proper way to do it:
dt = rbind(dt, dt, dt, dt)
dt$row = c(rep("top", 20), rep("bottom", 20))
dt$col = c(rep("left", 10), rep("right", 30))
p2 = ggplot(data = dt, aes(x = x, y = y)) + 
  geom_point() + 
  facet_wrap(row~col) +
  # theme_classic() + 
  theme(legend.position = "none")
egg::tag_facet(p2, open = "", close = "", tag_pool = c("A", "B", "C", "D"))
               # label.args = list(gp = grid::gpar(fontfamily = "sans", 
               #                             fontsize = 12, fontface = "bold"), 
               #                   x = unit(0, "cm"), vjust = 1, hjust = 1))

g1 = gtable_frame(ggplotGrob(p1), width = unit(1, "null"), height = unit(2, "null"), debug = TRUE)
g2 = gtable_frame(ggplotGrob(p1), width = unit(1, "null"), height = unit(1, "null"), debug = TRUE)
g3 = gtable_frame(ggplotGrob(p1), width = unit(1, "null"), height = unit(1, "null"), debug = TRUE)
g4 = gtable_frame(ggplotGrob(p1), width = unit(2, "null"), height = unit(1, "null"), debug = TRUE)
